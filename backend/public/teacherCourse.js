...
async function loadCourseDetails() {
  ...
  try {
    ...
    document.getElementById("course-details").innerHTML = `
      ...
      ${
        Array.isArray(course.Students) && course.Students.length > 0
          ? `<div class='students-resources-list'>${course.Students.map(
              (s) => `
            <div class="resource-row">
              <span class="resource-badge badge-a1">${
                s.Student_Name ? s.Student_Name.charAt(0).toUpperCase() : "?"
              }</span>
              <span class="resource-title">${s.Student_Name}</span>
              <span class="resource-members">${s._id}</span>
              <span class="resource-members grade">${
                s.Grade ? s.Grade : "-"
              }</span>
    ...
    document
      ...
      `;
        document
          .querySelector(".add-course-cancel-btn")
          .addEventListener("click", function () {
            detailsDiv.innerHTML = originalHTML;
          });
        document
          .getElementById("update-course-form")
          .addEventListener("submit", async function (e) {
            ...
            try {
              const res = await fetch(
                `/api/courses/updatecourse/${course._id}`,
                {
                  method: "PUT",
    ...
    document.querySelectorAll(".student-update-btn").forEach((btn, idx) => {
      btn.addEventListener("click", function () {
        ...
        row.innerHTML = `
          ...
          <input class="student-edit-name" type="text" value="${
            student.Student_Name
          }" style="flex:1; font-size:1rem; padding:4px 8px; border-radius:6px; border:1px solid #d1d5d8; margin-right:8px;">
          <span class="resource-members">${student._id}</span>
          <input class="student-edit-grade" type="text" value="${
            student.Grade ? student.Grade : ""
          }" style="width:48px; font-size:1rem; padding:4px 8px; border-radius:6px; border:1px solid #d1d5d8; margin-right:8px;">
          <span class="resource-members year">${
        ...
        row
          .querySelector(".student-save-btn")
          .addEventListener("click", async function () {
            const newName = row
              .querySelector(".student-edit-name")
              .value.trim();
            const newGrade = row
              .querySelector(".student-edit-grade")
              .value.trim();
            ...
            if (newName && newName !== student.Student_Name) {
              const res = await fetch(
                `/api/students/updatestudents/${student._id}`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
            ...
            if (newGrade && newGrade !== student.Grade) {
              ...
              const gradeDoc = grades.find(
                (g) => g.Student === student._id && g.Course === course._id
              );
              if (gradeDoc) {
                const res = await fetch(
                  `/api/grades/updategrade/${gradeDoc._id}`,
                  {
                    method: "PUT",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({
        ...
        row
          .querySelector(".student-cancel-btn")
          .addEventListener("click", function () {
            row.innerHTML = originalHTML;
          });
      });
      ...
      `;
        ...
        addRow
          ...
          .addEventListener("click", async function () {
            ...
            const email = addRow
              .querySelector(".student-add-email")
              .value.trim();
            const password = addRow
              .querySelector(".student-add-password")
              .value.trim();
            const grade = addRow
              .querySelector(".student-add-grade")
              .value.trim();
            ...
            try {
              await fetch("/api/grades/addnewgrade", {
                ...
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify([
                  { Student: studentId, Course: courseId, Grade: grade },
                ]),
              });
            } catch (err) {}
            alert("Student added!");
