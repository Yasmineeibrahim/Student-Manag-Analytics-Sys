window.addEventListener("DOMContentLoaded", async function () {
  ...
  function renderInfo() {
    document.getElementById("teacher-info").innerHTML = fields
      .map(
        (f) => `
      <div class="settings-row">
        <span class="settings-label">${f.label}:</span>
        <span class="settings-value" id="${f.key}">${
          f.key === "Password" ? "********" : info[f.key]
        }</span>
        <button class="settings-edit-btn" data-field="${
          f.key
  ...
  function renderEdit(field) {
    ...
    let row = document.querySelector(
      `.settings-edit-btn[data-field="${field}"]`
    ).parentElement;
    row.innerHTML = `
      <span class="settings-label">${f.label}:</span>
