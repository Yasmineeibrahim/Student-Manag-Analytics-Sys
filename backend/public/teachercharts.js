...
(async function () {
  ...
  document
    ...
    ?.addEventListener("click", async function (e) {
      ...
      try {
        const res = await fetch(`/api/courses/deletecourse/${courseId}`, {
          method: "DELETE",
        });
        if (res.ok) {
          courseRow.remove();
      ...
      courses.forEach((course) => {
        ...
        div.innerHTML = `
          <span class="resource-badge badge-a1">${
            course.Course_Code.split(" ")[0] || "N/A"
          }</span>
          ...
          <button class="resource-btn view" onclick="location.href='teacherCourse.html?id=${
            course._id
          }'">View Course</button>
          <button class="course-delete-btn" data-id="${
            course._id
          }" aria-label="Delete"><i class="fa-solid fa-trash" style="color: #a82929;"></i></button>
        `;
  ...
  try {
    ...
    const sortedStudents = studentsWithAvg
      .slice()
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  if (addCourseBtn && addCourseModal && closeModalBtn && addCourseForm) {
    addCourseBtn.addEventListener("click", function () {
      const teacherName = localStorage.getItem("teacherName") || "";
      const instructorInput = document.querySelector(
        '#add-course-form input[name="Instructor"]'
      );
      if (instructorInput) {
        ...
        if (res.ok) {
          ...
          if (teacherId) {
            ...
            if (teacher && teacher.Courses) {
              ...
              console.log("Attempting to update teacher with new course:", {
                teacherId,
                newCourseId,
                updatedCourses,
              });
              const updateRes = await fetch(
                `/api/teachers/updateteacher/${teacherId}`,
                {
                  method: "PUT",
                  headers: { "Content-Type": "application/json" },
    ...
    console.log("Modal or form elements not found:", {
      addCourseBtn,
      addCourseModal,
      closeModalBtn,
      addCourseForm,
